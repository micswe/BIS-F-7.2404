...
<#
	...
	.NOTES
		...
		10.08.2023 Micswe: Update ngen Prozess
	.LINK
		https://eucweb.com
#>
...
Begin {
 ...
 ELSE {
	...
	IF (($LIC_BISF_CLI_DotNet -eq "YES") -and ($CTXAppLayerName -eq "OS-Layer")) {
		...
		foreach ($element in $NgenPath) {
			...
			if ($element.contains("v4"))
			{
				### Executing Optimization 1 v3+
				$PrepCommands += [pscustomobject]@{
					Order = "$ordercnt";
					Enabled = "$true";
					showmessage = "Y";
			...
			if ($element.contains("v2"))
			{
				### Executing Optimization 2 -v2
				$PrepCommands += [pscustomobject]@{
					Order = "$ordercnt";
					Enabled = "$true";
					showmessage = "Y";
 ...
 ELSE {
	...
	function PreCommand {
		...
		Foreach ($prepCommand in ($PrepCommands | Sort-Object -Property "Order")) {
			...
			IF ( (($prepCommand.TestPath -eq "" ) -or (Test-Path $($prepCommand.TestPath))) -and $prepCommand.Enabled -eq "$true" ) {

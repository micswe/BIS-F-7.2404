<#
	...
	.NOTES
		...
		09.05.2024 MCISWE: AddCleanUP Sppol Folder after Reboot
...
Process {
	...
	IF (!($CTXAppLayerName -eq "OS-Layer")) {
		...
		IF ($LIC_BISF_CLI_MS_Spool_CleanUP -eq "YES") {
			Write-BISFlog -Msg "CleanUP Spool Folder " -showConsole -Color Cyan

			IF ($LIC_BISF_CLI_SPb -eq "1")
			{
				$Global:LIC_BISF_SpoolPath = "$PVSDiskDrive\$LIC_BISF_CLI_SpoolFolder"
				Write-BISFLog -Msg "CleanUP Spool Folder: $LIC_BISF_SpoolPath" -ShowConsole -Color DarkCyan -SubMsg				
				Remove-Item -Path "$LIC_BISF_SpoolPath\*" -Force
			}

		} ELSE {
			Write-BISFlog -Msg "CleanUP Spool Folder is NOT enabled in BIS-F GPO" -showConsole -Color Cyan
		}		
	}
	ELSE {
